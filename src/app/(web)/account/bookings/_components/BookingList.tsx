...
function Invoice() {
  const ref = React.createRef();  // Create a ref to capture the HTML
 
  const generatePdf = () => {
    const element = ref.current; // Get the HTML content to be converted

    // Set a fixed width for the element to avoid resizing based on the device width
    const originalWidth = element.style.width; // Save original width
    element.style.width = '895px'; // Set fixed width for the container used in the PDF

    html2canvas(element, { scale: 2 }).then((canvas) => {
      const pdf = new jsPDF('p', 'mm', 'a4'); // Portrait, A4 size (210mm x 297mm)

      // Set the aspect ratio for the canvas to avoid squishing
      const imgData = canvas.toDataURL('image/png');
      const imgWidth = 190; // Width in mm (adjust based on your content)
      const imgHeight = (canvas.height * imgWidth) / canvas.width; // Calculate height proportionally

      // Add the image to the PDF (scaled to fit A4 dimensions)
      pdf.addImage(imgData, 'PNG', 10, 10, imgWidth, imgHeight); // x=10, y=10, width=190mm, height calculated

      // Trigger the download
      pdf.save('invoice.pdf');
      
      // Clean up: reset the width after generating the PDF
      element.style.width = originalWidth; // Restore the original width
    });
  };

  return (
    <div className="absolute top-0 right-0 left-0 bottom-0 w-full h-full z-50">
      <button onClick={generatePdf} className="bg-blue-500 text-white p-2 rounded mb-4">
        Download Invoice
      </button>

      <>
        {/* Invoice */}
        <div ref={ref} className="w-full max-w-[895px]">
          <div className="s">
            {/* Card */}
